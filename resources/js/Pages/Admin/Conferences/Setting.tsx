...
import { Button, Grid, Image, Stack, Text, NumberInput, ColorInput, Container, Group, Title, Card } from '@mantine/core';
...
export default function ConferenceSetting() {
  ...
  const getInitialPositions = () => {
    ...
    // Default positions
    return {
      name: { x: 419, y: 297.5, size: 20, width: 400, color: '#000000', align: 'center', text: 'PARTICIPANT NAME' },
      paper_title: { x: 419, y: 325.5, size: 14, width: 700, color: '#000000', align: 'center', text: 'PAPER TITLE' }
    };
  };
  ...
  function handleFileSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    post(route('conferences.uploadCertificate', conference.data.id), {
      forceFormData: true,
      onSuccess: () => {
        notifications.show({ title: 'Success', message: 'Certificate template uploaded successfully!', color: 'green' });
  ...
  function handlePositionSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    router.post(route('conferences.uploadCertificate', conference.data.id), {
      ...
      onSuccess: () => {
        notifications.show({ title: 'Success', message: 'Template position saved successfully!', color: 'green' });
      },
      onError: (e) => {
        console.log('error', e);
      },
    ...
    <Container size="md" py="xl">
      <Stack gap="lg">
        <Group justify="space-between">
          <div>
            <Title order={2}>Certificate Settings</Title>
            <Text c="dimmed" size="sm">
              Manage certificate template and text positions
            </Text>
          </div>
          <Button
            variant="subtle"
        ...
        <Card padding="lg" radius="md" withBorder>
          ...
          {templateUrl && (
            <div style={{ marginTop: 32 }}>
              <Text fw={500} mb="md">Template Preview & Position Settings</Text>
              ...
              <Grid>
                ...
                <Grid.Col span={{ base: 12, md: 4 }}>
                  <form onSubmit={handlePositionSubmit}>
                    <Stack gap="sm">
                      <Text fw={600}>Participant Name Settings</Text>
                      <NumberInput
                        label="X Position"
                      ...
                      <Text fw={600} mt="md">Paper Title Settings</Text>
                      <NumberInput
                        label="X Position"
...
ConferenceSetting.layout = (page: React.ReactNode) => (
  <MainLayout title="Conference Settings">{page}</MainLayout>
);
