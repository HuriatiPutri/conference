                ...
                <div class="card card-primary card-outline">
                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table id="conferencesTable" class="table-bordered table-hover table-striped table">
                        <thead>
                          <tr>
                            <th>No</th>
                            <th>Nama</th>
                        ...
                        <tbody>
                          ...
                          @forelse($conferences as $conference)
                            <tr>
                              ...
                              <td>
                                {{ $conference->date ? \Carbon\Carbon::parse($conference->date)->format('d/m/Y') : '-' }}
                              </td> {{-- Format tanggal --}}
                              <td>
                                @if ($conference->cover_poster_path)
                                  <img src="{{ Storage::url($conference->cover_poster_path) }}"
                                    alt="Poster {{ $conference->name }}" class="cover-thumb img-thumbnail">
                                @else
                                  -
                                @endif
                              </td>
                              ...
                              <td class="text-center">
                                <a href="{{ route('home.conference.setting-certificate', $conference->public_id) }}"
                                  class="btn btn-secondary btn-sm" title="Setting Sertifikat"><i
                                    class="fas fa-certificate"></i></a>
                                <a href="{{ route('conference.show', $conference->public_id) }}"
                                  class="btn btn-info btn-sm" title="Lihat"><i class="fas fa-eye"></i></a>
                                <a href="{{ route('conference.edit', $conference->public_id) }}"
                                  class="btn btn-warning btn-sm" title="Edit"><i class="fas fa-edit"></i></a>
                                <form action="{{ route('conference.destroy', $conference->public_id) }}" method="POST"
                                  style="display:inline-block;">
                                  @csrf
                                  @method('DELETE')
                                  <button type="submit" class="btn btn-danger btn-sm" title="Hapus"
                                    onclick="return confirm('Apakah Anda yakin ingin menghapus konferensi ini?')"><i
                  ...
                  <div class="card-footer clearfix">
                    ...
                    <small class="text-muted float-right">Total: {{ $conferences->count() }} konferensi</small>
                  </div>
                </div>
              </div>
            </div>
